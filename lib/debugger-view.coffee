...
class DebuggerView extends View
  @content: ->
    @div class: 'atom-debugger', =>
      @header class: 'header', =>
        @span class: 'header-item title', 'SWD Debugger'
  ...
  initialize: (target, pid) ->
    gdb = atom.config.get("swd-debugger.gdbPath")
    # command = adb
    ...
    #@GDB.target 'remote localhost:3333', (clazz, result) ->
      ...
      unless frame = result.frame
        ...
        if frame.fullname
          ...
          if @exists(fullpath)
            ...
            @GDB.stack 'list-locals', '--all-values', (ok, data) =>
              if ok
